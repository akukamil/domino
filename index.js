...
var app, fbs, assets={},client_id, objects={}, state='',chat_path, game_tick=0, made_moves=0, game_id=0, my_turn=0, connected = 1,opponent=0, LANG = 0, hidden=0, game_platform="", git_src = '',room_name = 'room0', moving_chip=null, pending_player='',tm={}, some_process = {}, my_data={opp_id : ''},opp_data={}, game_name='domino';
...
music={
	...
	switch(){
		...
		if (this.on){
			this.on=0;
			assets.music.stop();
			objects.pref_info.text=['Музыка отключена','Music is off'][LANG];
...
keyboard={
	...
	read(max_symb){
		...
		anim2.add(objects.chat_keyboard_cont,{y:[450, objects.chat_keyboard_cont.sy]}, true, 0.4,'linear');	
	...
	close () {		
		...
		anim2.add(objects.chat_keyboard_cont,{y:[objects.chat_keyboard_cont.y,450]}, false, 0.4,'linear');		
...
pref={
	...
	music_switch_down(){
		...
		music.switch();
		sound.play('click3');
		const tar_x=music.on?130:91;//-39
	...
	sound_switch_down(){
		...
		sound.switch();
		sound.play('click3');
		const tar_x=sound.on?291:252;//-39
	...
	async update_bcg(){
		...
		if (!my_data.bcg_id){
			if (game.on) objects.bcg.texture=assets.bcg;
			return;
		}		
		...
		if (game.on) objects.bcg.texture=this.bcg_loader.resources[res_name].texture;
	},
...
lobby={
	...
	async inst_message2(t){
		...
		sound.play('locked');		
		anim2.add(objects.inst_msg_cont,{alpha:[0, 1]},true,0.4,'linear',false);		
		objects.inst_msg_avatar.texture=assets.pc_icon;
...
async function init_game_env(lang) {
	...
	app = new PIXI.Application({width:M_WIDTH, height:M_HEIGHT,antialias:false,backgroundColor : 0x202020,autoDensity:true,resolution:1.2});
